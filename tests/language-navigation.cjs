...
const ids=['language-home','language-babelian','panel-siren','language-back','babelian-backups','language-open-babelian','language-open-siren','babelian-title','siren-title'];
...
get('language-open-babelian').events.click();assert.equal(nav.current(),'babelian');assert(get('language-home').hidden);assert(!get('language-babelian').hidden);assert(get('panel-siren').hidden);assert(!get('babelian-backups').hidden);assert.equal(focused,'babelian-title');
...
assert.equal((source.match(/if\(\$\('language-babelian'\)\.hidden\)return;/g)||[]).length,2);
